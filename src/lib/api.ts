...
export async function addWedding(data: Omit<Wedding, 'id'>): Promise<number> {
  ...
  const { data: row, error } = await supabase
    ...
    .insert({
      ...
      venue: data.venue,
      city: data.city,
      total_budget: (data as Wedding & { totalBudget?: number }).totalBudget ?? null,
    })
    .select('id')
    .single()
  if (error) throw error
...
export async function updateWedding(id: number, data: Partial<Wedding>): Promise<void> {
  ...
  if ((data as Wedding & { totalBudget?: number }).totalBudget !== undefined)
    patch.total_budget = (data as Wedding & { totalBudget?: number }).totalBudget
...
export async function seedIfEmpty(): Promise<void> {
  ...
  // Ceremonies
  const ceremonies: Omit<Ceremony, 'id'>[] = [
    { weddingId: wId, name: 'Mehendi Ceremony', date: '12 Jun', time: '4:00 PM', location: "Bride's residence", guests: 80, side: "Bride's side", status: 'upcoming' },
    { weddingId: wId, name: 'Sangeet Night', date: '13 Jun', time: '7:00 PM', location: 'Jai Mahal Palace Lawns', guests: 220, side: 'Both sides', status: 'upcoming' },
    { weddingId: wId, name: 'Baraat Procession', date: '14 Jun', time: '10:00 AM', location: "Groom's house → Venue", guests: 150, side: "Groom's side", status: 'upcoming' },
    { weddingId: wId, name: 'Pheras (Main Ceremony)', date: '14 Jun', time: '12:00 PM', location: 'Mandap, Jai Mahal Palace', guests: 347, side: 'Both sides', status: 'upcoming' },
    { weddingId: wId, name: 'Vidaai', date: '14 Jun', time: '5:30 PM', location: 'Main Gate', guests: 347, side: 'Both sides', status: 'upcoming' },
    { weddingId: wId, name: 'Reception', date: '14 Jun', time: '7:00 PM', location: 'Banquet Hall', guests: 347, side: 'Both sides', status: 'upcoming' },
  ]
  await supabase.from('ceremonies').insert(
    ceremonies.map(c => ({ user_id: userId, wedding_id: c.weddingId, name: c.name, date: c.date, time: c.time, location: c.location, guests: c.guests, side: c.side, status: c.status }))
  )
  ...
  await supabase.from('vendors').insert(
    vendors.map(v => ({ user_id: userId, wedding_id: v.weddingId, name: v.name, category: v.category, city: v.city, amount: v.amount, pay_status: v.payStatus, phone: v.phone, detail: v.detail }))
  )
  ...
  const guests: Omit<Guest, 'id'>[] = [
    ...
    { weddingId: wId, name: 'Vikram Verma', side: 'Groom', relation: 'College friend', ceremonies: 'Baraat, Sangeet', rsvp: 'Awaited', transport: '—', food: 'Veg', phone: '9876500003', roomNumber: '', checkedIn: false },
    ...
    { weddingId: wId, name: 'Mohanlal Jain', side: 'Both', relation: 'Parivar mitra', ceremonies: 'Pheras, Reception', rsvp: 'No', transport: '—', food: 'Jain', phone: '', roomNumber: '', checkedIn: false },
    { weddingId: wId, name: 'Kavita Patel', side: 'Groom', relation: 'Bua ji', ceremonies: 'All', rsvp: 'Yes', transport: 'Coach B', food: 'Veg', phone: '9876500006', roomNumber: '', checkedIn: false },
    ...
    { weddingId: wId, name: 'Anita Mehta', side: 'Bride', relation: 'Padosan', ceremonies: 'Mehendi, Reception', rsvp: 'Awaited', transport: '—', food: 'Veg', phone: '9876500008', roomNumber: '', checkedIn: false },
  ]
  await supabase.from('guests').insert(
    guests.map(g => ({ user_id: userId, wedding_id: g.weddingId, name: g.name, side: g.side, relation: g.relation, ceremonies: g.ceremonies, rsvp: g.rsvp, transport: g.transport, food: g.food, phone: g.phone ?? '', room_number: g.roomNumber ?? '', checked_in: g.checkedIn ?? false }))
  )

  // Budget
  const budget: Omit<BudgetCategory, 'id'>[] = [
  ...
  await supabase.from('budget_categories').insert(
    budget.map(b => ({ user_id: userId, wedding_id: b.weddingId, category: b.category, spent: b.spent, total: b.total }))
  )
  ...
  await supabase.from('tasks').insert(
    tasks.map(t => ({ user_id: userId, wedding_id: t.weddingId, type: t.type, label: t.label, who: t.who, done: t.done }))
  )

  // Timeline
  const timeline: Omit<TimelineItem, 'id'>[] = [
    ...
    { weddingId: wId, day: 'pre', time: '12 Jun · 4:00 PM', text: 'Mehendi Ceremony 🌿', sub: "~80 guests · Bride's side", color: 'var(--teal)', sortOrder: 1 },
    { weddingId: wId, day: 'pre', time: '12 Jun · 8:00 PM', text: 'Mehendi dinner', sub: 'Family gathering', color: 'var(--teal)', sortOrder: 2 },
    { weddingId: wId, day: 'pre', time: '13 Jun · 2:00 PM', text: "Haldi — Bride's side 🌸", sub: 'Intimate · Family only', color: 'var(--purple)', sortOrder: 3 },
    { weddingId: wId, day: 'pre', time: '13 Jun · 3:00 PM', text: "Haldi — Groom's side 🌸", sub: "Groom's house · Friends + family", color: 'var(--amber)', sortOrder: 4 },
    { weddingId: wId, day: 'pre', time: '13 Jun · 7:00 PM', text: 'Sangeet Night 🎵', sub: 'Lawns · DJ · ~220 guests', color: 'var(--pink)', sortOrder: 5 },
  ]
  await supabase.from('timeline').insert(
    timeline.map(t => ({ user_id: userId, wedding_id: t.weddingId, day: t.day, time: t.time, text: t.text, sub: t.sub, color: t.color, sort_order: t.sortOrder }))
  )

  // Vendor Library
  const vendorLib: Omit<VendorLib, 'id'>[] = [
  ...
  await supabase.from('vendor_library').insert(
    vendorLib.map(v => ({ user_id: userId, name: v.name, category: v.category, city: v.city, phone: v.phone, detail: v.detail, rating: v.rating, used_in: v.usedIn }))
  )
}

...
export default function Guests({ weddingId, onAddGuest, refreshKey, onRefresh }: Props) {
  ...
  async function exportGuests() {
    ...
    const rows = gs.map(g => ({ Name: g.name, Side: g.side, Relation: g.relation, Ceremonies: g.ceremonies, RSVP: g.rsvp, Transport: g.transport, FoodPref: g.food }));
      ...
      <div className="card">
        <div className="card-hd">
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button className="btn btn-p btn-sm" onClick={onAddGuest}><i className="ti ti-plus" /> Add Guest</button>
          </div>
        </div>
        <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 8 }}>Upload Excel columns: Name, Side (Bride/Groom/Both), Relation, Ceremonies, RSVP (Yes/No/Awaited), Transport, FoodPref</div>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead>
              <tr><th>Name</th><th>Side</th><th>Relation</th><th>Ceremonies</th><th>RSVP</th><th>Transport</th><th>Food</th><th /></tr>
            </thead>
            <tbody>
              {filtered.map(g => (
                <tr key={g.id}>
                  ...
                  <td>{g.food || '—'}</td>
                  <td>
                    <button className="btn btn-sm" onClick={() => g.id && deleteGuest(g.id)} style={{ color: 'var(--coral)', border: 'none', padding: '3px 5px' }}>
